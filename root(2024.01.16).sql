/* 
 DB Handling
 SQL 명령을 통해서 DBMS 의 storage 에 저장된 데이터에 접근하여
 여러가지 연산을 수행하는 것
*/
--	저장소(Scheme, Schema)를 설계, 생성
--  저장소를 기준으로 데이터를 추가, 수정, 삭제, 조회
--	DDL(Data Definition Lang. 데이터 정의의 언어)
--  저장소를 생성, 삭제, 변경 하는 명령어 체계
--	DDL 명령의 생성하는 명령어는 "CREATE" 키워드로 시작
--	CREATE DATABASE, CREATE TABLE, CREATE USER (대표적인 생성 키워드)
--	물리적 저장소인 내부 Schema 생성하기 : schoolDB (1번만 실행)
...
-- 데이터를 CRUD 를 실행하기
-- 새로운(Table 에 없는) 데이터를 Table 에 추가하기
-- INSERT INTO [table] ([Columns]) VALUES (값들)

-- CREATE 생성하려면 INSERT INTO 입력.
INSERT INTO 
...
-- 필수항목(PK) 인 st_num 의 데이터가 준비되지 않아 데이터 추가 실패
INSERT INTO
tbl_student(st_name, st_dept)
VALUES('개원훈', '기계공학과');

-- 필수항목(NOT NULL) 인 st_name 의 데이터가 준비되지 않아 데이터 추가 실패
INSERT INTO
tbl_student(st_num, st_dept)
VALUES('S0003', '기계공학과');

-- 필수항목(NOT NULL) 인 st_dept 의 데이터가 준비되지 않아 데이터 추가 실패
INSERT INTO
...
-- table 에 추가되어(저장되어, 보관되어) 있는 DATA 를
...
-- tbl_student table 에서 모든 data를 가져오는데 
-- 원하는 데이터를 선택적으로 조회하기 (projection 설정하기)
SELECT st_num, st_name, st_dept
From tbl_student;

SELECT st_dept, st_grade,st_num,st_name
From tbl_student;

-- SELECTION 
--	어떤 조건을 부여하여 조건에 맞는 데이터들만 확인하고 싶을때
...
-- 데이터 변경하기
--	이미 추가된(Insert 된) 데이터의 특정 칼럼의 값을 변경하는 것(Update)
--	Update 명령을 잘못 입력하고, 실행을 하면 재앙이 올 수 있다.
--	UPDATE 명령에 WHERE 조건절이 생략되면 TABLE 에 있는 모든 데이터가 변경되어버린다.
--	그래서 반드시 UPDATE 명령은 WHERE 조건절이 필수이다.

-- st_name 에 조건을 부여하여 UPDATE 를 실행하고 있다.
-- st_name 칼럼은 데이터가 중복(동명이인) 될 수 있다.
-- 그렇기 때문에 아래와 같은 코드도 문제를 일으킬 소지가 매우 많다
UPDATE tbl_student
SET st_grade = 3
WHERE st_name = '강이찬';
...
--	2. SELECT 된 데이터 목록 중에서 Update 할 데이터의 PK 값을 확인한다.
...
INSERT INTO tbl_professor(pr_num, pr_name, pr_tel)
