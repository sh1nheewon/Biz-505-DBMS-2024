...
/*
AUTO_INCREMENT
PRIMARY KEY 이고, 정수형인 칼럼에 설정하는 키워드
데이터를 INSERT 할 때, 이 칼럼은 데이터를 포함하지 않는다.
...
INSERT tbl_iolist ( 
io_seq, io_date, io_time, io_pname, io_dname, io_dceo, io_div, io_quan, io_iprice, io_oprice
)
VALUES(
'1', '2023-01-03', '10:00:48', '버터링', 
'강릉농협하나로마트정동점', '강시우', '1', '62', '655','0');

INSERT INTO tbl_iolist ( 
io_seq, io_date, io_time, io_pname, io_dname, io_dceo, io_div, io_quan, io_iprice, io_oprice
)
VALUES(
'2', '2023-01-03', '10:03:29', '브라우니', 
'강원양돈농협성덕하나로마트', '갈성원' ,'2','68','0','1000'
);
...
-- xlxs csv로 저장 후 table에서 우클릭 후 table data import wizard, truncate 체크

-- tbl_iolist 테이블에 저장된 데이터의 개수 확인 : SELECT COUNT(*) *:칼럼이 들어가도 됨(ex.io_seq)
-- 데이터의 레코드수가 몇개인지
SELECT COUNT(io_seq)
FROM tbl_iolist;
...
-- LIMIT : 레코드를 원하는 개수의 레코드만 출력
SELECT * FROM tbl_iolist LIMIT 3;

-- LIMIT 3 OFFSET 10 : 10개를 건너뛰고 11번부터 3개만 출력
-- 처음부터 15개의 레코드를 건너뛰고 5개의 레코드만 보여주기
-- Pagination 에서 사용하는 코드
SELECT * FROM tbl_iolist LIMIT 5 OFFSET 15;
...
-- 시작 이상 종료 이하의 범위는 BETWEEN 키워드를 사용하여 검사.
...
-- 거래구분(io_div)이 2 이고 거래수량(io_quan)이 50개 이하인 데이터만 select
...
/*
	상품명(IO_PNAME)이 같은 값끼리 GROUP 을 만들고
    같은 GROUP 내에서 합산하여 결과를 보여주기.
*/
SELECT
	io_pname AS 상품명,
...
FROM tbl_iolist 
GROUP BY io_pname;
-- 상품명으로 그룹을 만들고 (GROUP BY) , 내부에서 같은 값의 합을 보여주기. SUM()

-- 상품별로 몇번 거래가 이루어졌는가?
-- 상품명으로 GROUP 을 만들고 GROUP 내부에서 개수를 세어서 보여라
...
-- 상품명으로 GROUP 을 설정하고 같은 상품 중에서 
-- 거래구분으로 분리하여 GROUP 설정.
