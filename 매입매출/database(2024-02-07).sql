...
실무에서 운영중인 Table 의 구조를 변경하는 것은 매우 신중해야 한다

구조를 변경하는 명령이 실행되면, 
사용중인 Table 은 Lock 이 걸리고
모든 CRUD 가 금지된다 
만약 활발하게 조회 등이 발생하는 Table 의 경우
다른 Transaction 의 영향으로 전체 시스템에 문제가 발생할 수 있다.
...
-- p_name 에 '초코' 라는 글자가 포함된 데이터들을 SELECT
SELECT * FROM tbl_products
WHERE p_name LIKE '%초코%';


-- like 연산자를 사용하여 SELECT 를 실행하는데
-- 조건이 없다(모든 데이터 가져오기)
SELECT * FROM tbl_products
WHERE p_name LIKE '%%';
